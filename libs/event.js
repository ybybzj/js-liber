define(['./lang','./array','./object','./function','./string'],function(L,ArrUtil,ObjUtil,FunUtil,S){
	...
	L.extend(EventHandler.prototype,{
		invoke:function(){
			...
			if(self.validators.length === 0 || 
				ArrUtil.some(this.validators,function(validator){
					var vFn = validator[1];
	...
	function once(event, fn, context) {
		...
		this.on(event, function fnc() {
		  ...
		  self.removeListener(event, fnc);
		}, context);
	}

	function removeListener(event, fn, validatorKey) {
		...
		ArrUtil.forEach(_getTypeList(events,event), function(evt){
			...
			if((index = ArrUtil.indexOf(events[evt].handlers,fn,0, function(target,handler){return target === handler.handler;})) !== -1){
	...
	function hasEvent(etype) {
		...
		ArrUtil.forEach(etype,function(typeStr){
			...
			var has = ArrUtil.some(ObjUtil.keys(events),function(evt){
				...
				return typeObj.typeStr === _typeObj.typeStr||
						(_typeObj.type === typeObj.type && 
						(typeObj.nsList.length === 0 ||
							 ArrUtil.some(typeObj.nsList,function(ns){
