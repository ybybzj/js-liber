define(['./native','./lang'],function(native,L){
    ...
    function _composeArrNativeFn(fnName,replace){
        ...
        return function() {
            ...
            if(nativeMethod) {
                ...
                return replace.apply(this,toArray(arguments).concat([len]));
            }
        };
    };
    var forEach = _composeArrNativeFn('forEach',function(obj,fn,context,len){
    ...
    var map = _composeArrNativeFn('map',function(obj,fn,context,len){
    ...
    var every = _composeArrNativeFn('every',function(obj,fn,context,len){
        if(this && this._chain){
            this._chain = false;
        }
        for(var index = 0; index < len; index++){
    ...
    var some = _composeArrNativeFn('some',function(obj,fn,context,len){
        if(this && this._chain){
            this._chain = false;
        }
        for(var index = 0; index < len; index++){
    ...
    var filter = _composeArrNativeFn('filter',function(obj,fn,context,len){
        var result = []
        for(var index = 0; index < len; index++){
            if(index in obj && fn.call(context,obj[index],index,obj)){
    ...
    var reduce = _composeArrNativeFn('reduce',function(obj,fn,memo,context,len){
        var result,index = 0;
        if(arguments<3 ) {
            if(len === 0){
    ...
    var reduceRight = _composeArrNativeFn('reduceRight',function(obj,fn,memo,context,len){
    ...
    var lastIndexOf = function(obj,target,fromIndex,fn,context){
        ...
        if(native['lastIndexOf'] && arguments <= 3){
    ...
    var _at = function(arr,index){
        var index = index < 0 ? arr.length + index : index;
        return arr[index];
    }
    ...
    var subtract = function(obj,arr,fn,context){
        ...
        fn = fn ? function(){return fn.call(this,arguments[1],arguments[0]);} : null;
        return reduce(obj,function(memo,item){
    ...
    var replace = function(obj,sub,finder,context) {
        var fn = L.isFunction(finder)?finder : function(item){return item === finder;};
